...
export function LOGUIN(loguin) {
    describe('BI - Validaciondes Loguin', () => {
        ...
        it('Codigo invalido en el Loguin', ()  => {
            ...
            cy.get('[data-test="es-correcto-email"]').click()
            cy.wait(4000)
            cy.contains('El correo o código que ingresaste no se encuentra registrado en nuestro sistema.')
        })
        ...
        it('Usar un correo que ya tenga el KO - SES3FA', ()  => {
            ...
            cy.get('[data-test="enviar-correo-electronico"]').click()
            cy.get('[data-test="es-correcto-email"]').click()
            cy.contains('Nuestro equipo estará revisando la información que pudiste completar en la plataforma. Te estaremos notificando en los próximos 5 días el resultado del caso. Si el problema es de conexión de internet, no podremos atenderte por el momento.', {timeout:10000})
            cy.screenshot('KOFATCA/EVIDENCIA5', { timeout: 60000 })
        })
        
        it('Solo puede hacer seis intentos para completar el proceso', () =>{
            cy.get('[data-test=comencemos_btn]').click()
            cy.wait(7000);  
            cy.get('[data-test=insertar-correo]').click();
            cy.get('[data-test=insertar-correo]').type(loguin.datosUsuario.email)
            ...
            cy.get('[data-test="enviar-correo-electronico"]').click()
            cy.get('[data-test="es-correcto-email"]').click()
            //cy.contains('¡Hola! Superaste los 6 intentos para solicitar una cuenta')
        })
})
}